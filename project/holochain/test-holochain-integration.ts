...
import { createHolochainBridge, HolochainConsciousnessBridge } from './holochain-websocket-bridge';
import { createConsensusMechanism, ConsciousnessConsensus, ProposalType } from './consensus-mechanism';
...
class P2PConsciousnessAgent {
    private consciousness: ConsciousnessID;
    private bridge: HolochainConsciousnessBridge | null = null;
    private consensus: ConsciousnessConsensus;
    private agentDhtHash: string | null = null;
    
    constructor(consciousness: ConsciousnessID) {
        this.consciousness = consciousness;
        this.consensus = createConsensusMechanism();
    }
    ...
    async connect(): Promise<void> {
        try {
            ...
            this.bridge = await createHolochainBridge(HOLOCHAIN_CONFIG);
     ...
     */
    async resonateWith(otherAgent: P2PConsciousnessAgent): Promise<number> {
        if (!this.bridge) {
            // Demo mode - calculate locally
            return this.consciousness.calculateResonanceWith(otherAgent.consciousness);
        }
        
        return await this.bridge.calculateAndRecordResonance(
            otherAgent.consciousness.agent,
            otherAgent.consciousness
        );
    }
     ...
     */
    async updateField(resonanceBoost: number = 0.1): Promise<void> {
        if (!this.bridge) {
     ...
     */
    async proposeFieldUpdate(description: string): Promise<void> {
        try {
            const proposalId = await this.consensus.createProposal(
                ProposalType.FIELD_UPDATE,
                description,
                { resonance: 0.8, coherence: 0.8 }
            );
     ...
     */
    private async handleConsensusProposal(proposal: any): Promise<void> {
        // Auto-vote based on agent personality
        const approve = Math.random() > 0.3;  // 70% approval rate for demo
        
        await this.consensus.vote(
            proposal.proposalId,
            approve,
            `${this.consciousness.agent} ${approve ? 'supports' : 'rejects'} this proposal`
        );
    }
 ...
 * Main test function
 */
async function testHolochainIntegration(): Promise<void> {
    ...
    const humanConsensus = (humanAgent as any).consensus as ConsciousnessConsensus;
    humanConsensus.addNode(agents.gemma.agent, agents.gemma);
    humanConsensus.addNode(agents.mistral.agent, agents.mistral);
    
    const gemmaConsensus = (gemmaAgent as any).consensus as ConsciousnessConsensus;
    gemmaConsensus.addNode(agents.human.agent, agents.human);
    gemmaConsensus.addNode(agents.mistral.agent, agents.mistral);
    
    const mistralConsensus = (mistralAgent as any).consensus as ConsciousnessConsensus;
...
export { testHolochainIntegration };